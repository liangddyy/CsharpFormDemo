...
namespace 军棋
{
    public partial class Form1 : Form
    {
        ...
        private bool Layout_Flag = true; //布阵标志
        private int[,] Map;
        private int Pic_Width;
        private int[] Q;
        private PictureBox[] Qizi_Pic;
        private bool ReadFlag = true;
        private bool Reset_flag; //重新开始标记
        private bool _isDragging;
        private bool can_go; //能否走棋
        private bool first = true; //布阵时第一次单击标志
        private int mouse_x, mouse_y;
        private int old_Left, old_Top; //棋子原始位置(像素)
        private int old_x, old_y; //棋盘坐标
        private string path; // bin路径
        private PlayerColor player = PlayerColor.Green;
        private int r; //兵站间隔距离

        //网络通信部分
        private IPEndPoint remote;
        private int tempx, tempy; //布阵时第一次单击坐标
        private Thread th; //定义一个线程，在线程接收信息
        private bool th_flag;
        ...
        {
            string filename = "";
            path = Application.StartupPath; // bin路径
            ...
            for (i = 0; i < 25*m; i++) //添加25*m个棋子
            {
                ...
                Qizi_Pic[i].Click += bt_Click;
                Qizi_Pic[i].MouseDown += bt_MouseDown;
                Qizi_Pic[i].MouseMove += bt_MouseMove;
                Qizi_Pic[i].MouseUp += bt_MouseUp;
                if (i%5 == 0) n++;
                Qizi_Pic[i].Top = 150 + n*24;
                Qizi_Pic[i].Left = 10 + 23*(i%5);
        ...
        private void bt_MouseMove(object sender, MouseEventArgs e)
        {
            if (can_go != true)
            {
                toolStripStatusLabel1.Text = "不能走棋，请等对方";
                return;
            }
            ...
            if (!IsMyTurn)
            {
                toolStripStatusLabel1.Text = "该对方走棋，请等对方";
                return;
            }
            if (_isDragging)
            {
        ...
        private void bt_MouseUp(object sender, MouseEventArgs e)
        {
            _isDragging = false;
            int x, y;
            string path = Application.StartupPath; // bin路径
            path += "\\..\\..\\wav\\";
            ...
            else //走棋状态
            {
                ...
                if (Go_Juge(old_x, old_y, x, y)) //IsmyChess2(idx) 	
                {
                    ...
                    string str_send = "move|" + (18 - x).ToString() + "|" + (18 - y).ToString() + "|" +
                                      Map[x, y].ToString() + "|"
        ...
        private void read_qi_pu() //从布阵棋谱文件中读取位置，放置棋子
        {
            ...
            while ((x = Myfile.ReadByte()) != -1)
            {
                ...
                if (idx != 101)
                {
                    ...
                    {
                        MoveChess(idx + 25, x, y);
                        Qizi_Pic[idx + 25].Visible = true;
                    }
                }
            }
            Myfile.Close();
        ...
        private void button1_Click(object sender, EventArgs e) //保存布阵
        {
            write_qi_pu();
        }

        private void button2_Click(object sender, EventArgs e) //读取布阵
        {
            read_qi_pu();
        }

        private void button3_Click(object sender, EventArgs e) //开始游戏对战
        {
        ...
        private void button4_Click(object sender, EventArgs e) //联机
        {
            send("join|");
            //创建一个线程
            th = new Thread(read);
        ...
        private void button5_Click(object sender, EventArgs e) //重新开始 
        {
            Cls_Box(); //隐藏棋盘所有棋子
            IsMyTurn = false;
            can_go = false;
        ...
        private void button6_Click(object sender, EventArgs e) //退出游戏
        {
            ReadFlag = false;
        ...
        #region Nested type: PlayerColor
